        ...
        Vector getColor(const Ray& R,const int n_rebond)
        {
            ...
            if (intersect(R, P, N, id, t))
            {
                if (id == 0)
                {
                    ...
                    return Vector(0,0,0);//intensite,intensite,intensite);
                }
...
int main() {
    ...
    Sphere     sph2(Vector (  0,   0, 0),    10,Vector (0.7,0.0,0.0));
    Sphere     sph3(Vector (-20 ,  0,  20),    10,Vector (0.0,0.7,0.0) );
    Sphere     sph4(Vector ( 20 ,  0, -20),    10,Vector (0.0,0.0,0.7) );
    ...
    for (int i = 0; i < H; i++) {
        ...
        for (int j = 0; j < W; j++) {
            ...
            {   
                ...
                double gx = 0.; //r*cos(2.*M_PI*r2)*1.;
                double gy = 0.; //r*sin(2.*M_PI*r2)*1.;

                double r1b = uniform(engine[tid]);
                double r2b = uniform(engine[tid]);
                double rb  = sqrt(-2.*log(r1b));
                double gxb = rb*cos(2.*M_PI*r2b)*1.;
                double gyb = rb*sin(2.*M_PI*r2b)*1.;

                u = Vector(j-W/2.+0.5+gx, H/2.-i-0.5+gy, -W/(2.*tan(fov/2.)));
                u.normalize();

                Vector Pprime = camera + u * dist_focus;
                Vector Oprime = camera + Vector(gxb,gyb,0);
                Vector dirPrime = Pprime-Oprime;
                dirPrime.normalize();

                Ray R(Oprime,dirPrime);
